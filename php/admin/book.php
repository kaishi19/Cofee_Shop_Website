...
<?php
    ...
    // client pending reservations or cancellation
    $clientPendingStatement = "SELECT `rc`.`gcash_ref` `gcash_reference`, `c`.`l_name` `client_lastname`, `c`.`f_name` `client_firstname`, `c`.`p_num` `client_phone`, `c`.`email` `client_email`, `r`.`tbl_id` `table_id`, `r`.`id` `reservation_id`, `r`.`time_in` `reservation_timein` , `r`.`time_out` `reservation_timeout`, `r`.`status` `reservation_status` FROM `rsv` `r` INNER JOIN `clt` `c` ON `r`.`clt_id` = `c`.`id` INNER JOIN `rct` `rc` ON `rc`.`rsv_id` = `r`.`id` WHERE `r`.`status` IN ('pending', 'cancelling_pending', 'cancelling_reserved');";
    $clientPendingResult = mysqli_query($conn, $clientPendingStatement);
...
?>
    ...
    <section class="rsvs sec-a">
        <div>
            <h1 class="rsvs-t tt-imgban">REQUESTS <?php echo (mysqli_num_rows($clientPendingResult) > 0) ? '('.mysqli_num_rows($clientPendingResult).')' : "" ?></h1>
        </div>
        <div class="rsvs-cdbx">
            <?php
                if (mysqli_num_rows($clientPendingResult) == 0) {
                    echo '
                    <div class="fxbx-b cd-a">
                        <p class="rsvs-p">
                            You have no existing requests!
                        </p>
                    </div>
                    ';
                } else {
                    foreach ($clientPendingResult as $clientPending) {
                        if ($clientPending["reservation_status"] == "cancelling_pending") {
                            $statusStatement = "Undo Reservation Attempt";
                        } else if ($clientPending["reservation_status"] == "cancelling_reserved") {
                            $statusStatement = "Cancellation Request";
                        } else if ($clientPending["reservation_status"] == "pending") {
                            $statusStatement = "Reservation Request";
                        } else {
                            $statusStatement = "";
                        }
                        echo '
                        <div class="cd">
                            <div class="cd-hdr">
                                <h3>Table #'.$clientPending["table_id"].'</h3>
                                <p>
                                    '.ucwords($statusStatement).'
                                </p>
                            </div>
                            <div>
                                <p class="client">
                                ...
                                <p> 
                                '.(ucwords($clientPending["client_firstname"]) ." ". ucwords($clientPending["client_lastname"])).'
                                </p>
                                <p class="client">
                                    Client Email Address
                                </p>
                                <p>
                                    '.$clientPending["client_email"].'
                                </p>
                                <p class="client">
                                    Client Contact Number
                                </p>
                                <p>
                                    +63'.$clientPending["client_phone"].'
                                </p>
                                <p class="gcash">
                                    GCash Reference Number
                                </p>
                                <p>
                                    '.$clientPending["gcash_reference"].'
                                </p>
                            </div>
                            <div>
                                <p class="reserve">
                                ...
                                <p>
                                    '.$clientPending["reservation_timein"].'
                                </p>
                                <p class="reserve">
                                    Reservation Time Out
                                </p>
                                <p>
                                    '.$clientPending["reservation_timeout"].'
                                </p>
                            </div>
                        ';
                        if ($clientPending["reservation_status"] == "cancelling_pending") {
                            $accept = "cancel_pending";
                            $deny = "uncancel_pending";
                        } else if ($clientPending["reservation_status"] == "cancelling_reserved") {
                            $accept = "cancel_reserved";
                            $deny = "uncancel_reserved";
                        } else {
                            $accept = "reserve";
                            $deny = "cancel";
                        }
                        echo 
                            '<div class="cd-fxbx">
                                <p>
                                    <a class="an-udl-ho" href="../includes/process.php?rsv_id='.$clientPending["reservation_id"].'&req='.$accept.'">Approve</a>
                                </p>
                                <p>
                                    <a class="an-udl-ho" href="../includes/process.php?rsv_id='.$clientPending["reservation_id"].'&req='.$deny.'">Reject</a>
                                </p>
                            </div>
                        </div>';
                    }
